...
一个*字符串常量*是一串在两个`U+0022`（双引号）字符中的任意Unicode字符，当字符是`U+0022`自身的时候，它必须在字符前跟一个`U+005C`字符（`\`）*转义*，或者使用*原始字符串常量*。

多行字符串常量可以通过在换行之前前跟一个`U+005C`字符（`\`）来结束每一行来定义。这会导致`U+005C`字符，换行符和所有下一行开头的空白都被忽略。
...
一个包含在括号中的表达式代表它它内部表达式的结果。括号可以用来显式的指定表达式内的计算顺序。
